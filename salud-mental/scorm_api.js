 ...
 * Encapsula las llamadas LMSInitialize/LMSSetValue/LMSGetValue/LMSCommit/LMSFinish
 * y provee una interfaz simple: setScore, setStatus, setSuspendData, getSuspendData, commit, isInitialized.
 */
(function (global) {
  'use strict';
  ...
  function locateAPI() {
    ...
    if (!found && window.parent && window.parent !== window) {
      found = findAPI(window.parent);
    }
    if (!found && window.top && window.top !== window) {
      found = findAPI(window.top);
    }
  ...
  function init() {
    ...
    api = locateAPI();
    if (!api) {
      console.warn('[AacromSCORM] No LMS API found — running in standalone mode.');
      return false;
    }
  ...
  function safeSet(key, value) {
    ...
    try {
      var rc = api.LMSSetValue(key, String(value));
      return rc === 'true' || rc === true;
  ...
  function setStatus(status) {
    safeSet('cmi.core.lesson_status', status);
  }
